...
* 函数名: LoadWaveFile(...)
*   功能: 读取波形文件的数据
*   注意: 当bReadData参数的值为真时，将分配一个pData指针指向
*         的声音数据缓冲区。使用后必须释放它
*             (c)  jk&ljq, 2010
****************************************************************/
...
*   功能: 创建一个次级缓冲区,并且将filename指向的声音数据
*         调入该缓冲区
*             (c)  jk&ljq, 2010
****************************************************************/
BOOL CDSound::CreateStaticBuffer( DWORD dwIndex, LPCSTR filename )
{
    ...
    // 锁定缓冲区，以便向它写入数据
	hr = ppBuffer[dwIndex]->Lock(0,0,&lpvPtr,&dwBytes,NULL,NULL,
...
*             (c)  jk&ljq, 2010
****************************************************************/
DWORD  CDSound::GetStaticFreq  ( DWORD dwIndex)
{
	// 检测有效性
    if( !bSoundAble )             return 0;
    if( dwIndex >= dwNumStatics ) return 0;
    if( ppBuffer == NULL )        return 0;
    if( ppBuffer[dwIndex]==NULL ) return 0;
    DWORD dwFreq;
	if( ppBuffer[dwIndex] )
    {
        ppBuffer[dwIndex]->GetFrequency( &dwFreq );
...
*             (c)  jk&ljq, 2010
****************************************************************/
LONG CDSound::GetStaticVolume( DWORD dwIndex)
{
		// 检测有效性
    if( !bSoundAble )             return 0;
    if( dwIndex >= dwNumStatics ) return 0;
	...
	if( ppBuffer[dwIndex] )
    {
        ppBuffer[dwIndex]->GetVolume( &Volume );
	}
	return Volume;
}
/***************************************************************
...
BOOL CDSound::CreateStreamBuffer( DWORD dwIndex, LPCSTR filename )
{
	...
	hr = lpDSound->CreateSoundBuffer( &dsbd,
                                      &ppStream[dwIndex]->lpBuffer,
                                      NULL);
    if( FAILED( hr )) {
        delete ppStream[dwIndex];
        ppStream[dwIndex] = NULL;
    ...
    ppStream[dwIndex]->lSecondsLen = (LONG) (
        ppStream[dwIndex]->lDataLen /
        ppStream[dwIndex]->wfx.nAvgBytesPerSec );
    // 估算缓冲区的一半的时间长度(以毫秒为单位)
    ppStream[dwIndex]->lMillisecsHalfBuf =(LONG)(
        (FLOAT)ppStream[dwIndex]->lBufSize * 500.0F/
        (FLOAT)ppStream[dwIndex]->wfx.nAvgBytesPerSec );
...
*     注: 调用本函数时，只有当达到更新缓冲区开头部分或
*         结尾部分的数据的条件后，才会从文件读取数据。
*         每次从文件读取数据后，文件数据的播放游标自动
*         下移；如果是循环播放文件数据则游标自动回绕。
*         前后两次从文件读取数据的时间间隔由流式缓冲描
*         述成员lMillisecsHalfBuf决定。
*             (c)  jk&ljq, 2010
****************************************************************/
...
/***************************************************************
* 函数名: StopStream(...)
*   功能: 停止流式缓冲区的播放，并将缓冲区和文件数据的
*         播放游标置于开头。然后将文件数据的开头部分载
*         入缓冲区。所以，调用StopStream一次则从文件读
*         取数据一次；使用时要避免频繁的使用StopStream，
*         以免程序的运行速率变慢。
*             (c)  jk&ljq, 2010
****************************************************************/
