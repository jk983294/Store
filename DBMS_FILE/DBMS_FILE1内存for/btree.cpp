...
{
	cout<<"block number"<<current.num<<"    data count"<<current.count<<" data:";
...
void BTree::insert_value(record re1)     //添加记录
{
	//添加记录  首先查找是否存在该记录  并将路径记录在path栈中
	bool isExist=get_path(re1.phone_number);
	if (isExist)
	{
		cout<<"该节点已存在"<<endl;
		return; 
        ...
        current=path_node.top();//获取最后一个结点所在的块号
		path_node.pop();   
		...
		node parent1;
		int left,right;
		for (*re=re1,parent1=current,left=-1,right=-1;;)
		{
		...
		if (pos==M)   //若插入的位置在中间则将插入的结点的值作为父亲结点
		{
			...
			write_block(current.num);  //协会磁盘
            conf.count++;   //块数加1
			initNode(); //重新生成一个结点
			current.num=conf.count-1;
            current.count=M;
			//开始赋值
			for (i=0;i<M;i++)
			{
			...
			{
				initNode();
				current.num=conf.count;
				current.count=1;
				conf.count++;
				current.r[0]=*re;
				current.block_ptr[0]=parent1.num;
			...
			{
				root =path_node.top();  //得到下一个根结点
		        path_node.pop();
                //split_node(re,root,parent1.num,conf.count-1);  
				*re=*re;
				left=parent1.num;
				parent1=root;
			//	left=parent1.num;
		...
		else if (pos<M)   //则m-1位置上的记录作为父亲结点
		{
			...
			*parent=current.r[M-1];
			...
			else
			{
				root =path_node.top();  //得到下一个根结点
				path_node.pop();
                //split_node(*parent,root,parent1.num,conf.count-1);   
				*re=*parent;
				left=parent1.num;
				parent1=root;
		...
		else
		{
			...
			write_block(current.num);  //写回块的信息
			break;
	...
	/*	if (current.count==2*M)  //若结点已满
		{
			//分裂结点
			split_node(re,current,-1,-1);  //记录，父亲
...
/*void BTree::split_node(record re,node parent1,int left,int right)     //分裂结点
{
	node root;
	//cout<<path.size()<<endl;
